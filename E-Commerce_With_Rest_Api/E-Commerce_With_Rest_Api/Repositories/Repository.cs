...
namespace E_Commerce_With_Rest_Api.Repositories
{
    ...
    {
        protected E_CommerceApiContext context = new E_CommerceApiContext();
        ...
        {
            return context.Set<OrderProduct>().Where(x=> x.CustomerID == cid && x.ProductHistoryId == pid).ToList();
        }

        public bool GetByProductName(string name, int id)
        {
            if (context.Set<Product>().FirstOrDefault(c => c.MainCategoryId == id) != null && context.Set<Product>().FirstOrDefault(c => c.Product_name == name) != null)
            {
                return true;
            }
            else
        ...
        {
            return context.Set<ProductHistory>().Where(x => x.MainCategoryId==id && x.Product_name == name).FirstOrDefault();
        ...
        {
            return context.Set<Product>().Where(p => p.MainCategoryId == mid && p.SaleID == null).ToList();
        }
        ...
        {
            var item = context.Set<Product>().Find(product.ProductId);
        ...
        {
            var item = context.Set<ProductHistory>().Find(product.ProductHistoryId);
            item.ImageFile = product.ImageFile;
            item.Description = product.Description;
            item.UnitPrice = product.UnitPrice;
            item.Product_name = product.Product_name;
